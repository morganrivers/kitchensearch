...
Search runs entirely on your machine. Your search queries and embeddings are never sent anywhere, and there is no analytics or usage tracking. It's also cool to have tiny embedding models running locally. Two things do make network requests: viewing a combo fetches its image over HTTPS from Google's gstatic servers (so Google sees which combos you view, but never your search text), and if you choose to activate a license key, the app contacts the Polar payment service to validate it, sending the key and a random per-install ID. After activation it re-validates with Polar about once every 30 days when the app starts. Nothing is sent unless you activate a license. 
